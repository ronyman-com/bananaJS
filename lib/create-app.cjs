const fs = require('fs-extra');
    ...
    const normalizePath = (inputPath) => {
      // This normalization might need adjustment depending on exact path requirements
      // For now, keeping the original logic but be aware of potential issues with drive letters/UNC paths
      return path.normalize(inputPath); // Simplified normalization
    };
    ...
    module.exports = async function createApp(appName, template, options = {}) {
      ...
      // Determine the parent directory for the new app.
      // Use the provided parentPath from options, falling back to cwd if not provided.
      const parentDir = normalizePath(options.parentPath || process.cwd());

      // Construct the full path for the new application directory by joining the parent directory and the app name.
      const appDir = path.join(parentDir, appName); // <-- This is the corrected line
    ...
    console.log(`✅ Banana Successfully created ${template} app for ${appName}`);



      try {
        // Create directory structure
        await fs.ensureDir(appDir); // This will now create the directory inside parentDir
        const files = await fs.readdir(templateDir);

        // Process files with progress tracking
        await Promise.all(files.map(async (file) => {
          const src = path.join(templateDir, file);
          const dest = path.join(appDir, file); // Destination is correctly inside appDir
        ...
        return {
          appDir, // Return the correctly calculated appDir
          template,
          framework: template,
          success: true
        };
